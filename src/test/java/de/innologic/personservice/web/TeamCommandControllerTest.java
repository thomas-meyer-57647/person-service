...
class TeamCommandControllerTest {
    ...
    void shouldCreateTeam_whenValidRequest() throws Exception {
        when(teamCommandService.createTeam(eq("1"), any(TeamCreateRequest.class), eq("actor-1")))
                .thenReturn(sampleTeamResponse("team-20", "1"));

        mockMvc.perform(post("/api/v1/companies/{companyId}/teams", "1")
    ...
    void shouldUpdateTeam_whenValidPatch() throws Exception {
        when(teamCommandService.updateTeam(eq("1"), eq("team-20"), any(TeamUpdateRequest.class), eq("actor-1")))
                .thenReturn(sampleTeamResponse("team-20", "1"));

        mockMvc.perform(patch("/api/v1/companies/{companyId}/teams/{teamId}", "1", "team-20")
                        .header("X-Actor-Id", "actor-1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
    ...
    void shouldReturn404_whenUpdateTeamNotFound() throws Exception {
        when(teamCommandService.updateTeam(eq("1"), eq("team-404"), any(TeamUpdateRequest.class), eq("actor-1")))
                .thenThrow(new NotFoundException("Team not found"));

        mockMvc.perform(patch("/api/v1/companies/{companyId}/teams/{teamId}", "1", "team-404")
                        .header("X-Actor-Id", "actor-1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
    ...
    void shouldTrashTeam_whenTeamExists() throws Exception {
        TeamResponse trashed = sampleTeamResponse("team-20", "1");
        trashed.setTrashedAt(LocalDateTime.now());
        trashed.setTrashedBy("actor-1");
        when(teamCommandService.trashTeam("1", "team-20", "actor-1")).thenReturn(trashed);

        mockMvc.perform(post("/api/v1/companies/{companyId}/teams/{teamId}/trash", "1", "team-20")
                        .header("X-Actor-Id", "actor-1"))
                .andExpect(status().isOk())
                .andExpect(header().string("Content-Type", containsString("json")))
                .andExpect(jsonPath("$.teamId").value("team-20"))
    ...
    void shouldReturn404_whenTrashTeamNotFound() throws Exception {
        when(teamCommandService.trashTeam("1", "team-404", "actor-1")).thenThrow(new NotFoundException("Team not found"));

        mockMvc.perform(post("/api/v1/companies/{companyId}/teams/{teamId}/trash", "1", "team-404")
                        .header("X-Actor-Id", "actor-1"))
                .andExpect(status().isNotFound())
                .andExpect(header().string("Content-Type", containsString("json")))
                .andExpect(jsonPath("$.errorCode").value("NOT_FOUND"))
                .andExpect(jsonPath("$.message").value(containsString("Team not found")))
                .andExpect(jsonPath("$.path").value("/api/v1/companies/1/teams/team-404/trash"));
    }

    @Test
    void shouldRestoreTeam_whenTeamExists() throws Exception {
        TeamResponse restored = sampleTeamResponse("team-20", "1");
        restored.setTrashedAt(null);
        restored.setTrashedBy(null);
        when(teamCommandService.restoreTeam("1", "team-20", "actor-1")).thenReturn(restored);

        mockMvc.perform(post("/api/v1/companies/{companyId}/teams/{teamId}/restore", "1", "team-20")
                        .header("X-Actor-Id", "actor-1"))
                .andExpect(status().isOk())
                .andExpect(header().string("Content-Type", containsString("json")))
                .andExpect(jsonPath("$.teamId").value("team-20"))
    ...
    void shouldReturn404_whenRestoreTeamNotFound() throws Exception {
        when(teamCommandService.restoreTeam("1", "team-404", "actor-1")).thenThrow(new NotFoundException("Team not found"));

        mockMvc.perform(post("/api/v1/companies/{companyId}/teams/{teamId}/restore", "1", "team-404")
                        .header("X-Actor-Id", "actor-1"))
                .andExpect(status().isNotFound())
                .andExpect(header().string("Content-Type", containsString("json")))
                .andExpect(jsonPath("$.errorCode").value("NOT_FOUND"))
                .andExpect(jsonPath("$.message").value(containsString("Team not found")))
                .andExpect(jsonPath("$.path").value("/api/v1/companies/1/teams/team-404/restore"));
    }

    @Test
    void shouldAddMember_whenValidRequest() throws Exception {
        TeamMemberResponse response = sampleMemberResponse("membership-50", "1", "team-20", "person-10");
        when(teamCommandService.addTeamMember(eq("1"), eq("team-20"), any(TeamMemberAddRequest.class), eq("actor-1")))
                .thenReturn(response);

        mockMvc.perform(post("/api/v1/companies/{companyId}/teams/{teamId}/members", "1", "team-20")
                        .header("X-Actor-Id", "actor-1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                ...
                .andExpect(status().isCreated())
                .andExpect(header().string("Content-Type", containsString("json")))
                .andExpect(jsonPath("$.membershipId").value("membership-50"))
                .andExpect(jsonPath("$.companyId").value("1"))
                .andExpect(jsonPath("$.teamId").value("team-20"))
                .andExpect(jsonPath("$.personId").value("person-10"));
    }

    @Test
    void shouldReturn400_whenAddMemberJoinedAtInFuture() throws Exception {
        mockMvc.perform(post("/api/v1/companies/{companyId}/teams/{teamId}/members", "1", "team-20")
                        .header("X-Actor-Id", "actor-1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                  "personId": "person-10",
                                  "joinedAt": "2100-01-01T00:00:00"
                                }
                                """))
                .andExpect(status().isBadRequest())
                .andExpect(header().string("Content-Type", containsString("json")))
                .andExpect(jsonPath("$.errorCode").value("VALIDATION_ERROR"))
                .andExpect(jsonPath("$.message").value(containsString("joinedAt")))
                .andExpect(jsonPath("$.path").value("/api/v1/companies/1/teams/team-20/members"));
    }
    ...
    @Test
    void shouldReturn409_whenPrimaryTeamAlreadyAssigned() throws Exception {
        when(teamCommandService.addTeamMember(eq("1"), eq("team-20"), any(TeamMemberAddRequest.class), eq("actor-1")))
                .thenThrow(new ConflictException("Person already has an active primary membership"));
        ...
        mockMvc.perform(post("/api/v1/companies/{companyId}/teams/{teamId}/members", "1", "team-20")
                ...
                .andExpect(header().string("Content-Type", containsString("json")))
                .andExpect(jsonPath("$.errorCode").value("CONFLICT"))
                .andExpect(jsonPath("$.message").value(containsString("primary membership")))
                .andExpect(jsonPath("$.path").value("/api/v1/companies/1/teams/team-20/members"));
    }

    @Test
    void shouldRemoveMember_whenExistingMembership() throws Exception {
        mockMvc.perform(delete("/api/v1/companies/{companyId}/teams/{teamId}/members/{personId}", "1", "team-20", "person-10")
                        .header("X-Actor-Id", "actor-1"))
    ...
    void shouldReturn404_whenRemoveMemberNotFound() throws Exception {
        doThrow(new NotFoundException("Active team member relation not found"))
                .when(teamCommandService).removeTeamMember("1", "team-20", "person-404", "actor-1");

        mockMvc.perform(delete("/api/v1/companies/{companyId}/teams/{teamId}/members/{personId}", "1", "team-20", "person-404")
                        .header("X-Actor-Id", "actor-1"))
                .andExpect(status().isNotFound())
                .andExpect(header().string("Content-Type", containsString("json")))
                .andExpect(jsonPath("$.errorCode").value("NOT_FOUND"))
                .andExpect(jsonPath("$.message").value(containsString("Active team member relation not found")))
                .andExpect(jsonPath("$.path").value("/api/v1/companies/1/teams/team-20/members/person-404"));
    }
    ...
    private TeamMemberResponse sampleMemberResponse(String membershipId, String companyId, String teamId, String personId) {
